...
import { auditEvents, organizations, user } from "@iq/db";
...
export default async function Audit() {
  ...
  const rows = await db.select({ a: auditEvents, actor: user.name, organization: organizations.name }).from(auditEvents)
    .leftJoin(user, eq(user.id, auditEvents.actorId))
    .leftJoin(organizations, sql`${auditEvents.targetType} = 'organization' and ${auditEvents.targetId} = ${organizations.id}::text`)
          ...
          <TableBody>
            {rows.map(({ a, actor, organization }) => (
              <TableRow key={a.id} className="align-top">
                ...
                <TableCell className={organization ? undefined : "font-mono text-xs"}>{organization ?? `${a.targetType} ${a.targetId.slice(0, 8)}`}</TableCell>
